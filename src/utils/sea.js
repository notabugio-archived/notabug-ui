...
(function () {
  ...
  USE(function (module) {
    ...
    if (!api.crypto) {
      try {
        ...
        const {
          subtle
        } = USE('@trust/webcrypto', 1); // All but ECDH
  ...
  USE(function (module) {
    const SEA = USE('./root');
    const Buffer = USE('./buffer');
    const settings = {}; // Encryption parameters
    ...
    const authsettings = Object.assign({}, _initial_authsettings); // This creates Web Cryptography API compliant JWK for sign/verify purposes
  ...
  USE(function (module) {
    ...
    const {
      pbkdf2
    } = USE('./settings'); // This internal func returns SHA-256 hashed data for signing
  ...
  USE(function (module) {
    ...
    SEA.work = SEA.work || (async (data, pair, cb, opt) => {
      try {
        ...
        const key = await (shim.ossl || shim.subtle).importKey('raw', new shim.TextEncoder().encode(data), {
  ...
  USE(function (module) {
    ...
    SEA.pair = SEA.pair || (async (cb, opt) => {
      try {
        const ecdhSubtle = shim.ossl || shim.subtle; // First: ECDSA keys for signing/verifying...

        var sa = await shim.subtle.generateKey(S.ecdsa.pair, true, ['sign', 'verify']).then(async keys => {
          // privateKey scope doesn't leak out from here!
          //const { d: priv } = await shim.subtle.exportKey('jwk', keys.privateKey)
          const key = {};
          key.priv = (await shim.subtle.exportKey('jwk', keys.privateKey)).d;
          const pub = await shim.subtle.exportKey('jwk', keys.publicKey); //const pub = Buff.from([ x, y ].join(':')).toString('base64') // old
        ...
        try {
          var dh = await ecdhSubtle.generateKey(S.ecdh, true, ['deriveKey']).then(async keys => {
            // privateKey scope doesn't leak out from here!
            const key = {};
            key.epriv = (await ecdhSubtle.exportKey('jwk', keys.privateKey)).d;
            const pub = await ecdhSubtle.exportKey('jwk', keys.publicKey); //const epub = Buff.from([ ex, ey ].join(':')).toString('base64') // old
  ...
  USE(function (module) {
    ...
    SEA.sign = SEA.sign || (async (data, pair, cb, opt) => {
      try {
        ...
        const sig = await (shim.ossl || shim.subtle).importKey('jwk', jwk, S.ecdsa.pair, false, ['sign']).then(key => (shim.ossl || shim.subtle).sign(S.ecdsa.sign, key, new Uint8Array(hash))); // privateKey scope doesn't leak out from here!
  ...
  USE(function (module) {
    ...
    var parse = USE('./parse');
    var u;
    var seaverify = !!/seaverify/.test(window.location.search);
    SEA.verify = SEA.verify || (async (data, pair, cb, opt) => {
      try {
        const json = parse(data);
        if (!seaverify) pair = false;
        if (false === pair) {
          // don't verify!
          const raw = json !== data ? json.s && json.m ? parse(json.m) : data : json;
  ...
  USE(function (module) {
    ...
    SEA.encrypt = SEA.encrypt || (async (data, pair, cb, opt) => {
      try {
        ...
        const ct = await aeskey(key, rand.s, opt).then(aes =>
        /*shim.ossl ||*/
        shim.subtle.encrypt({
  ...
  USE(function (module) {
    ...
    SEA.decrypt = SEA.decrypt || (async (data, pair, cb, opt) => {
      try {
        ...
        const json = parse(data);
        const ct = await aeskey(key, shim.Buffer.from(json.s, 'utf8'), opt).then(aes =>
        /*shim.ossl ||*/
        shim.subtle.decrypt({
          // Keeping aesKey scope as private as possible...
          name: opt.name || 'AES-GCM',
          iv: new Uint8Array(shim.Buffer.from(json.iv, 'utf8'))
  ...
  USE(function (module) {
    ...
    SEA.secret = SEA.secret || (async (key, pair, cb, opt) => {
      try {
        ...
        const props = Object.assign(S.ecdh, {
          public: await ecdhSubtle.importKey(...pubKeyData, true, [])
        });
        const privKeyData = keysToEcdhJwk(epub, epriv);
        const derived = await ecdhSubtle.importKey(...privKeyData, false, ['deriveKey']).then(async privKey => {
          // privateKey scope doesn't leak out from here!
          const derivedKey = await ecdhSubtle.deriveKey(props, privKey, {
            name: 'AES-GCM',
            length: 256
  ...
  ;
  USE(function (module) {
    var shim = USE('./shim'); // Practical examples about usage found from ./test/common.js
  ...
  USE(function (module) {
    ...
    User.prototype.create = function (alias, pass, cb, opt) {
      ...
      act.c = function (pair) {
        act.pair = pair || {}; // the user's public key doesn't need to be signed. But everything else needs to be signed with it!
      ...
      act.e = function (epub) {
        ...
        }, act.proof, act.f); // to keep the private key safe, we AES encrypt it with the proof of work!
      };
  ...
  USE(function (module) {
    const SEA = USE('./sea');
    const Gun = SEA.Gun; // After we have a GUN extension to make user registration/login easy, we then need to handle everything else.
