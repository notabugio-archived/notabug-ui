...
import { multiTopic, singleAuthor, repliesToAuthor, sortThings } from "../queries";
  ...
  routes: [
    ...
    basicQueryRoute({
      ...
      query: query((scope, { match: { authorId, type, sort, tab1, tab2 } }) =>
        repliesToAuthor(
          scope,
          { repliesToAuthorId: authorId ? `~${authorId}` : null, type: type === "all" ? null : type }
        )
          .then(thingSouls => sortThings(scope, { sort, thingSouls, tabulator: `~${tab1}.${tab2}` }))
          .then(things => serializeListing(things.slice(0, LISTING_SIZE))))
    }),
    basicQueryRoute({
      ...
      query: query((scope, { match: { authorId, type, sort, tab1, tab2 } }) =>
        singleAuthor(
          scope,
          { authorId: authorId ? `~${authorId}` : null, type: type === "all" ? null : type }
        )
          .then(thingSouls => sortThings(scope, { sort, thingSouls, tabulator: `~${tab1}.${tab2}` }))
          .then(things => serializeListing(things.slice(0, LISTING_SIZE))))
    })
...
const serializeListing = things => ({ ids: things.map(prop("id")).filter(id => !!id).join("+") });
